...
#' 
#' Plots an x-y, time-x, time-y track of a potential migration process and 
#' prompts the user to click on the figure to obtain initial estimates of range 
#' centroids and timing of start and end of migrations. 
#'
#' @param time time (can be a \code{\link[base]{POSIXt}})
#' @param x x and y coordinates.  Can be two separate vectors OR a complex "x" 
#' OR a two-column matrix/date-frame. 
#' @param y see x
...
#' @seealso [quickfit()], \code{\link{locator}}
#' @export
locate_shift <- function(time, x, y, n.clust = 2, ...){
  ...
  if(n.clust == 2){
		plot(x,y,asp=1, type="o", pch=19, col=rgb(0,0,0,.5), 
		     cex=0.5, main = "click on two centroids ", ...)
		points(x[c(1, length(x))], y[c(1, length(x))], col= c("green", "red"), 
		       pch = c(19, 20), cex=2)
		xy <- locator(2)
		
		plot(time,x, type="o", pch=19, col=rgb(0,0,0,.5), xaxt="n", xlab="", cex=0.5, 
		     main = "click on migration start and end times", ...)
